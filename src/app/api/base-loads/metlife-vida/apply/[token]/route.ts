...
export async function POST(request: NextRequest, { params }: { params: Promise<{ token: string }> }) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/base-loads/metlife-vida/apply/${encodeURIComponent(token)}`, {
            method: 'POST',
        ...
        return new Response(await response.text(), {
            ...
            headers: {
                ...
                'X-Taiico-Route': 'base-load-vida-apply',
            },
        });
    } catch (error) {
        console.error('MetLife Vida base load apply proxy failed', error);
