...
import { AlertTriangle, Calculator, CheckCircle2, Download, FileSpreadsheet, Loader2, Plus, Upload, UsersRound, X } from 'lucide-react';
import {
  applyCommissionImport,
  reviewCommissionExceptions,
  createCommissionPeriod,
  exportCommissionImportPreview,
...
export function CobranzaProspectadoresView({ initialWorkspace }: { initialWorkspace: CommissionWorkspace }) {
  ...
  async function reviewExceptions() {
    ...
    setBusy('review'); setError(''); setSuccess('');
    try {
      const result = await reviewCommissionExceptions(periodId);
      setSuccess(`Revisión terminada: ${result.resolved} excepciones resueltas y ${result.pending} pendientes.`);
      await loadDetail(periodId);
      setWorkspace((current) => {
        const previous = current.periods.find((period) => period.id === periodId)?.exception_count || 0;
        return { ...current,
          periods: current.periods.map((period) => period.id === periodId ? { ...period, exception_count: result.pending } : period),
          summary: { ...current.summary, pending_exceptions: current.summary.pending_exceptions - previous + result.pending },
        };
      });
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'No se pudieron revisar las excepciones'); }
    finally { setBusy(''); }
  }
  ...
  async function exportPreview() {
    ...
    try { await exportCommissionImportPreview(preview.token); }
    catch (caught) { setError(caught instanceof Error ? caught.message : 'No se pudo exportar la vista previa'); }
    finally { setBusy(''); }
  }
