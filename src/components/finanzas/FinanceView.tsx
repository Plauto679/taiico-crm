...
export function FinanceView({ initialOverview }: { initialOverview: FinanceOverview }) {
  ...
  return <div className="mx-auto flex h-full min-h-0 w-full max-w-[1600px] flex-col gap-4 overflow-hidden px-3 py-4 text-slate-900 sm:px-5 md:py-6">
    ...
    <div className="min-h-0 flex-1 overflow-auto rounded-2xl bg-white/95 p-4 shadow-xl md:p-6">
      ...
      {tab === 'Clasificación' && <Rules items={rules} onRefresh={() => load('Clasificación')} setError={setError} />}
    </div>
...
function MovementEditor({ item, onClose, onSave }: { item: FinanceMovement; onClose: () => void; onSave: (payload: Record<string, unknown>) => Promise<void> }) {
  const [category, setCategory] = useState(item.categoria); const [subcategory, setSubcategory] = useState(item.subcategoria); const [saving, setSaving] = useState(false);
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4"><div className="w-full max-w-xl rounded-2xl bg-white p-6 shadow-2xl"><div className="flex justify-between"><div><h2 className="text-lg font-bold">Clasificar movimiento</h2><p className="mt-1 text-sm text-slate-500">{item.descripcion_original}</p></div><button onClick={onClose}><X /></button></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold">Categoría<input value={category} onChange={(e) => setCategory(e.target.value)} className="mt-1 w-full rounded-lg border px-3 py-2 font-normal" /></label><label className="text-sm font-semibold">Subcategoría<input value={subcategory} onChange={(e) => setSubcategory(e.target.value)} className="mt-1 w-full rounded-lg border px-3 py-2 font-normal" /></label></div><div className="mt-6 flex justify-end gap-2"><button onClick={onClose} className="rounded-lg border px-4 py-2">Cancelar</button><button disabled={saving || !category.trim()} onClick={async () => { setSaving(true); await onSave({ categoria: category.trim(), subcategoria: subcategory.trim() || null }); setSaving(false); }} className="rounded-lg bg-emerald-600 px-4 py-2 font-bold text-white disabled:opacity-50">Guardar</button></div></div></div>;
}

function Recurring({ items, onDecide }: { items: RecurringGroup[]; onDecide: (fingerprint: string, status: string) => Promise<void> }) {
  return <div className="space-y-3"><div><h2 className="text-lg font-bold">Detección explicable de recurrentes</h2><p className="text-sm text-slate-500">La decisión humana se conserva y prevalece sobre futuras sugerencias.</p></div>{items.length ? items.map((item) => <div key={item.fingerprint} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border p-4"><div><p className="font-semibold">{item.label}</p><p className="text-xs text-slate-500">{item.company} · {item.occurrences} movimientos en {item.months} meses · promedio {money.format(item.average_amount)}</p><p className="mt-1 text-xs text-slate-400">{item.basis}</p></div><div className="flex items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${item.status === 'confirmado' ? 'bg-emerald-100 text-emerald-700' : item.status === 'descartado' ? 'bg-slate-100 text-slate-600' : 'bg-amber-100 text-amber-700'}`}>{item.status}</span><button onClick={() => onDecide(item.fingerprint, 'confirmado')} className="rounded-lg border border-emerald-300 px-3 py-1.5 text-xs font-bold text-emerald-700">Confirmar</button><button onClick={() => onDecide(item.fingerprint, 'descartado')} className="rounded-lg border px-3 py-1.5 text-xs font-bold">Descartar</button></div></div>) : <Empty>No hay patrones recurrentes suficientes con la información indexada.</Empty>}</div>;
}
...
function BudgetQuickForm({ company, onCreated }: { company: 'TLA' | 'TS'; onCreated: () => Promise<void> }) {
  const [category, setCategory] = useState(''); const [amount, setAmount] = useState(''); const month = new Date().toISOString().slice(0, 7) + '-01';
  return <div className="w-full max-w-md"><p className="mb-3">Crea la primera partida del mes para {company}.</p><div className="flex gap-2"><input value={category} onChange={(e) => setCategory(e.target.value)} placeholder="Categoría" className="min-w-0 flex-1 rounded-lg border px-3 py-2 text-slate-900" /><input value={amount} onChange={(e) => setAmount(e.target.value)} type="number" placeholder="Monto" className="w-28 rounded-lg border px-3 py-2 text-slate-900" /><button disabled={!category || !amount} onClick={async () => { await upsertBudget({ company, month, category, amount: Number(amount) }); await onCreated(); }} className="rounded-lg bg-emerald-600 px-3 py-2 font-bold text-white disabled:opacity-40">Guardar</button></div></div>;
}
...
function Rules({ items, onRefresh, setError }: { items: Rule[]; onRefresh: () => Promise<void>; setError: (value: string) => void }) {
  const [open, setOpen] = useState(false); const [preview, setPreview] = useState<{ rule: Rule; total: number; conflicts: number } | null>(null);
  return <div className="space-y-4"><div className="flex justify-between"><div><h2 className="text-lg font-bold">Reglas de clasificación</h2><p className="text-sm text-slate-500">Previsualiza coincidencias y conflictos antes de aplicar. La fuente original no se modifica.</p></div><button onClick={() => setOpen(true)} className="flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-bold text-white"><Plus className="h-4 w-4" />Regla</button></div>{items.length ? <div className="space-y-2">{items.map((item) => <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border p-4"><div><p className="font-semibold">{item.priority}. {item.name}</p><p className="text-xs text-slate-500">{item.company || 'Todas'} · {item.field} {item.operator.replace('_', ' ')} “{item.value}” → {item.exclusion ? 'Excluir' : `${item.category}${item.subcategory ? ` / ${item.subcategory}` : ''}`}</p></div><div className="flex gap-2"><button onClick={async () => { const result = await previewRule(item.id); setPreview({ rule: item, total: result.total, conflicts: result.conflicts }); }} className="rounded-lg border px-3 py-1.5 text-xs font-bold">Previsualizar</button><button onClick={async () => { try { const result = await revertRule(item.id); setError(`Se restauraron ${result.restored} movimientos.`); } catch (reason) { setError(reason instanceof Error ? reason.message : 'No hay una ejecución reversible'); } }} className="rounded-lg border px-3 py-1.5 text-xs font-bold">Revertir última</button><button onClick={async () => { await deleteRule(item.id); await onRefresh(); }} className="rounded-lg p-2 text-red-600"><Trash2 className="h-4 w-4" /></button></div></div>)}</div> : <Empty>No hay reglas configuradas.</Empty>}{open && <RuleModal onClose={() => setOpen(false)} onCreated={async () => { setOpen(false); await onRefresh(); }} />}{preview && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4"><div className="w-full max-w-md rounded-2xl bg-white p-6"><h3 className="text-lg font-bold">Previsualización</h3><p className="mt-3 text-sm"><strong>{preview.total}</strong> movimientos coinciden; <strong>{preview.conflicts}</strong> ya tienen una categoría diferente.</p><p className="mt-2 text-xs text-slate-500">Aplicar actualiza la capa de clasificación y queda registrado en auditoría.</p><div className="mt-5 flex justify-end gap-2"><button onClick={() => setPreview(null)} className="rounded-lg border px-4 py-2">Cancelar</button><button onClick={async () => { try { const result = await applyRule(preview.rule.id); setPreview(null); setError(`Regla aplicada a ${result.updated} movimientos.`); } catch (reason) { setError(reason instanceof Error ? reason.message : 'No se pudo aplicar'); } }} className="rounded-lg bg-emerald-600 px-4 py-2 font-bold text-white">Aplicar</button></div></div></div>}</div>;
}

function RuleModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => Promise<void> }) {
  const [name, setName] = useState(''); const [value, setValue] = useState(''); const [category, setCategory] = useState('');
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4"><div className="w-full max-w-lg rounded-2xl bg-white p-6"><div className="flex justify-between"><h2 className="text-lg font-bold">Nueva regla</h2><button onClick={onClose}><X /></button></div><div className="mt-5 space-y-3"><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nombre" className="w-full rounded-lg border px-3 py-2" /><input value={value} onChange={(e) => setValue(e.target.value)} placeholder="Texto contenido en la descripción" className="w-full rounded-lg border px-3 py-2" /><input value={category} onChange={(e) => setCategory(e.target.value)} placeholder="Categoría a asignar" className="w-full rounded-lg border px-3 py-2" /></div><div className="mt-5 flex justify-end gap-2"><button onClick={onClose} className="rounded-lg border px-4 py-2">Cancelar</button><button onClick={async () => { await createRule({ name, priority: 100, field: 'descripcion_original', operator: 'contiene', value, company: null, category, subcategory: null, enabled: true, exclusion: false }); await onCreated(); }} disabled={!name || !value || !category} className="rounded-lg bg-emerald-600 px-4 py-2 font-bold text-white disabled:opacity-40">Crear</button></div></div></div>;
}

function UploadWizard({ onClose, onPublished }: { onClose: () => void; onPublished: () => Promise<void> }) {
  const [source, setSource] = useState('tla_amex'); const [file, setFile] = useState<File | null>(null); const [preview, setPreview] = useState<{ ingestion_id: string; rows: number; new_rows: number; duplicates: number } | null>(null); const [busy, setBusy] = useState(false); const [error, setError] = useState('');
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4"><div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl"><div className="flex items-start justify-between border-b p-6"><div><h2 className="text-xl font-bold">Carga guiada de estados</h2><p className="text-sm text-slate-500">Previsualiza, detecta duplicados y confirma antes de tocar el histórico canónico.</p></div><button onClick={onClose}><X /></button></div><div className="space-y-4 p-6"><ErrorMessage text={error} /><label className="block text-sm font-semibold">Fuente<select value={source} onChange={(e) => { setSource(e.target.value); setPreview(null); }} className="mt-1 w-full rounded-lg border px-3 py-2 font-normal"><option value="tla_amex">TLA · AMEX</option><option value="tla_bbva">TLA · BBVA</option><option value="tla_banorte">TLA · Banorte</option><option value="ts_bbva">TS · BBVA</option></select></label><label className="block rounded-xl border-2 border-dashed border-slate-300 p-6 text-center"><Upload className="mx-auto h-7 w-7 text-slate-400" /><span className="mt-2 block text-sm font-semibold">{file?.name || 'Seleccionar CSV canónico o estado original'}</span><input type="file" accept=".csv,.pdf" className="sr-only" onChange={(e) => { setFile(e.target.files?.[0] || null); setPreview(null); }} /></label>{preview && <div className="grid grid-cols-3 gap-3"><Kpi label="Filas" value={String(preview.rows)} /><Kpi label="Nuevas" value={String(preview.new_rows)} tone="green" /><Kpi label="Duplicadas" value={String(preview.duplicates)} tone={preview.duplicates ? 'amber' : 'green'} /></div>}<div className="rounded-lg bg-slate-50 p-3 text-xs text-slate-600">Los PDF originales se preservan, pero solo se publican automáticamente cuando existe un parser validado para ese formato. El sistema nunca adivina movimientos bancarios.</div></div><div className="flex justify-end gap-2 border-t p-5"><button onClick={onClose} className="rounded-lg border px-4 py-2">Cancelar</button>{!preview ? <button disabled={!file || busy} onClick={async () => { if (!file) return; setBusy(true); setError(''); try { setPreview(await previewIngestion(source, file)); } catch (reason) { setError(reason instanceof Error ? reason.message : 'No se pudo previsualizar'); } finally { setBusy(false); } }} className="rounded-lg bg-slate-900 px-4 py-2 font-bold text-white disabled:opacity-40">{busy ? 'Validando…' : 'Previsualizar'}</button> : <button disabled={busy || preview.duplicates > 0 || preview.new_rows === 0} onClick={async () => { setBusy(true); setError(''); try { await publishIngestion(preview.ingestion_id); await onPublished(); } catch (reason) { setError(reason instanceof Error ? reason.message : 'No se pudo publicar'); } finally { setBusy(false); } }} className="rounded-lg bg-emerald-600 px-4 py-2 font-bold text-white disabled:opacity-40">Confirmar y publicar</button>}</div></div></div>;
}
