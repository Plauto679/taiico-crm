    ...
    return [
        {
            ...
            "issue_summary": f"MetLife Vida renewal sheet is missing required columns: {', '.join(missing)}",
        }
    ]
...
def normalize_row(row_number: int, source_payload: dict[str, Any], today: datetime.date) -> ParsedRenewalCandidate:
    policy_number = normalize_policy_number(source_payload.get("POLIZA_ACTUAL"))
    original_policy_number = normalize_policy_number(source_payload.get("POLIZA_ORIGEN"))
    effective_start_date = parse_date(source_payload.get("INI_VIG"))
    renewal_deadline = parse_date(source_payload.get("FIN_VIG"))
    paid_until_date = parse_date(source_payload.get("PAGADO_HASTA"))
    premium_amount = parse_money(source_payload.get("PRIMA_ANUAL"))
    ...
    normalized_payload = {
        ...
        "rfc": clean_cell(source_payload.get("RFC_CONTRATANTE")),
        "plan_code": clean_cell(source_payload.get("FPLAN")),
        "product_name": clean_cell(source_payload.get("PDESC")),
        "effective_start_date": effective_start_date,
        "renewal_deadline": renewal_deadline,
        "days_until_renewal": days_until_renewal,
        "risk_level": calculate_risk_level(days_until_renewal),
        "payment_frequency_source": clean_cell(source_payload.get("FORMA_PAGO")),
        "policy_status_source": clean_cell(source_payload.get("ESTATUS_POL")),
        "collection_channel": clean_cell(source_payload.get("CONDUCTO_COBRO")),
        "promotoria": clean_cell(source_payload.get("PROMOTORIA")),
        "agent_code": clean_cell(source_payload.get("AGENTE")),
        "agent_name": clean_cell(source_payload.get("NOM_AGENTE")),
        "premium_amount": premium_amount,
        "modal_premium_amount": parse_money(source_payload.get("PRIMA_MODAL")),
    ...
    if not policy_number:
        issues.append({
            ...
            "issue_summary": f"MetLife Vida row {row_number} is missing POLIZA_ACTUAL.",
        })
    if renewal_deadline is None:
        issues.append({
            "severity": "high",
            "issue_type": "missing_renewal_deadline",
            "issue_summary": f"MetLife Vida row {row_number} is missing or has invalid FIN_VIG.",
        })
    if document_status == "missing":
        issues.append({
            "severity": "normal",
            "issue_type": "missing_expediente_link",
            "issue_summary": f"MetLife Vida row {row_number} has no EXPEDIENTE link.",
        })
...
def parse_metlife_vida_renewal_workbook(path: str | Path, today: datetime.date | None = None) -> tuple[list[ParsedRenewalCandidate], list[dict[str, str]]]:
    ...
    if "Vida" not in excel.sheet_names:
        return [], [
            {
                "severity": "critical",
                "issue_type": "missing_sheet",
                "issue_summary": "Workbook is missing required sheet: Vida",
            }
        ]

    df = pd.read_excel(workbook_path, sheet_name="Vida", dtype=object)
